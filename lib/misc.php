 ...
 * @param array 	$data					Additional arguments passed to the template.
 * @param string 	$template_path			Path to templates (template folder).
 * @param string	$default_path			Default path to template files (plugin folder).
 */
function uamswp_get_template_part($slug, $name = null, $data = [], $tempate_path = '', $default_path = '') {
    // here we're copying more of what get_template_part is doing.
    $template_names = [];
    $name = (string) $name;
	
	/* Order: Template => Default => Nothing */
    if ('' !== $tempate_path ) {
		if ('' !== $name ){
        	$template_names[] = $tempate_path . "/" . "{$slug}-{$name}.php";
		}
		$template_names[] = $tempate_path . "/" . "{$slug}.php";
	}

	if ('' !== $default_path) {
		if ('' !== $name ){
        	$template_names[] = $default_path . "/" . "{$slug}-{$name}.php";
		}
    	$template_names[] = $default_path . "/" . "{$slug}.php";
	}
	...
	// Loop through templates to find first option
