...
(function ($) {
	$.fn.responsiveCollapse = function(options) {
		...
		return this.filter('ul').each(function() {
			...
			var maxWidth     = $(container).width() - ($(container).find('.navbar-header').width() || 0); // the width of the grandparent of the main list
			...
			function grow() {
				var overflowWidth = $(overflowItem).width(); // width of overflow LI. Why not outerWidth?
				var child         = $(overflowList).children(); // Get the children of the overflow list.
				var count         = child.length; // the number of children of the overflow list

				if (count > 0) { // if there is at least one child.
					for (var i = 0; i < count; i++) {
						// Statement 1 sets a variable before the loop starts (var i = 0).
						// Statement 2 defines the condition for the loop to run (i must be less than the number of children of the overflow list).
						// Statement 3 increases a value (i++) each time the code block in the loop has been executed.

						// move the first dropdown item to end of list
						$(child[i]).insertBefore($list.children(':last-child')); // move the first item in the child array (the first child of the overflow) to the next to last position on the main list.
						...
						if((i == count - 1 && thisWidth - overflowWidth > maxWidth) || (i != count - 1 && thisWidth > maxWidth)) {
							// if i equals one less than the number of children of the overflow list
								// and if the list's outer width minus overflowWidth is greater than the width of the grandparent of the main list
							// or
							// if i does not equal one less than the number of children of the overflow list
								// and the list's outer width is greater than the width of the grandparent of the main list
